...
class carte:
    ...
    def mouvement(self, coov2, speed):
        ...
        if distanceX + distanceY < speed:
            cSpeed = speed - (distanceX + distanceY)
        pourcentX = distanceX / (distanceX + distanceY)
        pourcentY = distanceY / (distanceX + distanceY)
        mouvementX = int(pourcentX * cSpeed)
        mouvementY = int(pourcentY * cSpeed)
        signeX = 1
        signeY = 1
        if x2 < x:
            signeX = -1
        if y2 < y:
            signeY = -1
        x = x + mouvementX * signeX
        y = y + mouvementY * signeY
        self.coo = x, y
...
while obj.rn:
    ...
    ecran.screen.blit(pak.image_booster.ymage, (var_x // 2, var_y // 2))
    pygame.display.flip()
