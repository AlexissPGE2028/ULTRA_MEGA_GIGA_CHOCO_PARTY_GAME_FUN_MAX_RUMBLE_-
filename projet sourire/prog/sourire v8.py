...
def calcul_new_itineraire(coo, paket):
...
def dernier_carte_effect_cool(paket, index, speed):
...
def creer_nuage(liste, position, n):
    if obj.wait == 1:
        for i in range(n):
            liste.append(Particle(position, 10, random.randint(10, 20) * 0.3, random.randint(0, 360), None,
                                  image.chemin_nuage_effet))
...
def condition_dispawn_booster(pak_booster):
    if pak_booster.n_carte == pak_booster.numero:
        ...
        fini = effet_nuage(obj.liste_effect)
        if fini is True:
            return True
    else:
        affichageutilte(pak)
...
def affichageutilte(paket_booster):
    for pomme in paket_booster.liste:
        if pomme.cooV2 is not None:
            pomme.mouvement(20)
            pomme.image_dos.resize((var_x//3, var_x//3))
...
class image:
    ...
    def __init__(self, ymage=None):
        self.ymage = pygame.image.load(ymage).convert_alpha(ecran.screen)
...
class carte:
    ...
    def __init__(self, index=None, coo=None):
        ...
        self.cooV2 = None
        self.image = None
        self.image_dos = image(chemin_carte_dos)
        self.reverse = False
    ...
    def revel_carte(self, speed):
        if self.reverse is False:
            ...
            if calcul == 1:
                self.reverse = True
                self.image.resize((var_x//3, var_x//3))
                print(self.image.size_original)
